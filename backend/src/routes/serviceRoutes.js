...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (!services || services.length === 0) {
      ...
      await Service.insertMany(defaultServicesData);
      services = await Service.find();
      console.log('Services seeded. Count:', services.length);
    }
...
// GET /services/providers - fetch service providers by service and subservice
router.get('/providers', async (req, res) => {
  try {
    const { service, subservice } = req.query;
    
    console.log('Received request for service:', service, 'subservice:', subservice);
    
    if (!service) {
      return res.status(400).json({ message: 'Service parameter is required' });
    }

    // For now, return all providers to fix the immediate issue
    // We can refine the filtering logic later once we understand the data structure
    const allProviders = await ServiceProvider.find({}).select('-password');
    console.log('Total providers in database:', allProviders.length);
    
    if (allProviders.length === 0) {
      return res.json({ 
        message: 'No service providers are currently available',
        providers: []
      });
    }

    // Filter providers that have the requested service in their services array
    const filteredProviders = allProviders.filter(provider => {
      if (!provider.services || provider.services.length === 0) return false;
      
      return provider.services.some(providerService => {
        // Check if the service matches (case-insensitive)
        const serviceMatch = providerService.category && 
          providerService.category.toLowerCase() === service.toLowerCase();
        
        if (!subservice) return serviceMatch;
        
        // If subservice is specified, check subservices array
        if (serviceMatch && providerService.subservices) {
          return providerService.subservices.some(sub => 
            sub.name && sub.name.toLowerCase() === subservice.toLowerCase()
          );
        }
    ...
    console.log('Filtered providers found:', filteredProviders.length);

    if (filteredProviders.length === 0) {
      return res.json({ 
        message: 'No service providers are currently available for this service',
        providers: []
      });
    }
    ...
    res.status(500).json({ message: 'Failed to fetch service providers', error: err.message });
  }
});

// POST /services/book - create a new service request
router.post('/book', async (req, res) => {
  try {
    ...
    if (!userId || !providerId || !service || !address || !scheduledFor) {
      return res.status(400).json({ 
        message: 'Missing required fields: userId, providerId, service, address, scheduledFor' 
      });
    }
    ...
    const newRequest = new Request({
      ...
      subservice,
      address,
      decription: description, // Note: keeping the typo from your model
    ...
    // Populate the request with user and provider details
    const populatedRequest = await Request.findById(savedRequest._id)
      .populate('userId', 'name email phone')
      .populate('providerId', 'name email phone services');

    // Send notification email to provider (best-effort, don't block on failures)
    (async () => {
      try {
        // Find provider email from populatedRequest
        const provider = populatedRequest.providerId;
        const user = populatedRequest.userId;
        const providerEmail = provider?.email;

        if (!providerEmail) {
          console.log('No provider email available, skipping email.');
          return;
        }

        // Create transporter using SMTP env variables if provided, else Ethereal for testing
        let transporter;
        if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
          ...
          // fallback to ethereal account for local testing
        ...
        const scheduled = populatedRequest.scheduledFor ? new Date(populatedRequest.scheduledFor).toLocaleString() : 'Not specified';

        const mailOptions = {
          from: process.env.EMAIL_FROM || 'no-reply@fixitnow.example',
          to: providerEmail,
          subject: `New service request: ${populatedRequest.service}${populatedRequest.subservice ? ' - ' + populatedRequest.subservice : ''}`,
          text: `Hello ${provider.name || 'Service Provider'},\n\nYou have received a new service request from ${user?.name || 'a user'} (Email: ${user?.email || 'N/A'}, Phone: ${user?.phone || 'N/A'}).\n\nService: ${populatedRequest.service}${populatedRequest.subservice ? ' - ' + populatedRequest.subservice : ''}\nScheduled for: ${scheduled}\nAddress: ${populatedRequest.address}\nDetails: ${populatedRequest.decription || populatedRequest.description || 'No additional details provided.'}\n\nPlease log in to your FixItNow provider dashboard to accept or decline the request.\n\nBest regards,\nFixItNow Team`,
          html: `<p>Hello ${provider.name || 'Service Provider'},</p>
                 <p>You have received a new service request from <strong>${user?.name || 'a user'}</strong></p>
                 <ul>
                   <li><strong>Email:</strong> ${user?.email || 'N/A'}</li>
                   <li><strong>Phone:</strong> ${user?.phone || 'N/A'}</li>
                   <li><strong>Service:</strong> ${populatedRequest.service}${populatedRequest.subservice ? ' - ' + populatedRequest.subservice : ''}</li>
                   <li><strong>Scheduled for:</strong> ${scheduled}</li>
                   <li><strong>Address:</strong> ${populatedRequest.address}</li>
                   <li><strong>Details:</strong> ${populatedRequest.decription || populatedRequest.description || 'No additional details provided.'}</li>
                 </ul>
                 <p>Please log in to your FixItNow provider dashboard to accept or decline the request.</p>
                 <p>Best regards,<br/>FixItNow Team</p>`
        };

        const info = await transporter.sendMail(mailOptions);
        console.log('Provider notification email sent:', info.messageId);
        if (nodemailer.getTestMessageUrl && info) {
          console.log('Preview URL:', nodemailer.getTestMessageUrl(info));
        }
      } catch (emailErr) {
        console.error('Error sending provider email:', emailErr);
      }
    })();
